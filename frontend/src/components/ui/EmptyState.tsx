...
export function EmptyState({ icon: Icon, title, message, action }: Props) {
    return (
        <div className="flex flex-col items-center justify-center text-center py-16 px-4 animate-fade-up">
            <div className="w-20 h-20 rounded-[22px] flex items-center justify-center mb-5" style={{ background: 'linear-gradient(135deg, #b2dfdb, #ccfbf1)', border: '1.5px solid #e2e8f0' }}>
            ...
            {action && (
                <Link href={action.href}
                      ...
                      style={{ background: 'linear-gradient(135deg, #00897b, #00796b)', boxShadow: '0 6px 24px rgba(0,137,123,0.18)' }}>
                    {action.label}
                </Link>
            )}
        </div>
